    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.w_bits > 0:  # always true for llama 1
            self.lora_lsq_s = torch.nn.Parameter(torch.ones(int(out_features * in_features / gs)), requires_grad=False)
            self.lora_lsq_b = torch.nn.Parameter(torch.zeros(int(out_features * in_features / gs)), requires_grad=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.r == 0 or self.merged:
            ...
            # Init lsq step s
                ...
                return F.linear(x, q_weight, self.linear.bias) + lora
