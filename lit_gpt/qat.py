...
from lit_gpt.quant import WLSQPlus, WLSQPlusQKV


class LoRALayer(nn.Module):
    def __init__(self, r: int, lora_alpha: int, lora_dropout: float,
                 lsq: bool, w_bits: int, q_granul: str, gs: int):
        ...
        self.lsq = lsq
        self.w_bits = w_bits
        self.q_granul = q_granul
        self.gs = gs
        self.init_state = 0
        self.Qn = - 2 ** (w_bits - 1)
        self.Qp = 2 ** (w_bits - 1) - 1
        self.div = self.Qp if self.w_bits==4 else self.Qp + 1
...
class LoRALinear(LoRALayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout,
                         lsq=lsq, w_bits=w_bits, q_granul=q_granul, gs=gs)
        self.init_state = 0
        self.Qn = - 2 ** (w_bits - 1)
        self.Qp = 2 ** (w_bits - 1) - 1
        self.div = self.Qp if self.w_bits==4 else self.Qp + 1
        ...
        if r > 0:
            ...
            self.l_scale = self.lora_alpha / self.r
            self.reset_parameters()

        # lsq parameters
        if self.w_bits > 0:  # always true for llama 1
            self.lora_lsq_s = torch.nn.Parameter(torch.ones(int(out_features * in_features / gs)))
            self.lora_lsq_b = torch.nn.Parameter(torch.zeros(int(out_features * in_features / gs)))
            self.g = 1.0/math.sqrt(in_features * abs(self.Qp) / self.gs)
        # self.g = 1.0/math.sqrt(out_features * in_features * self.Qp)
    ...
    def merge(self) -> None:
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.w_bits == 0:
                self.linear.weight.data += (self.lora_B @ self.lora_A) * self.l_scale
                self.merged = True
            else:
                weight = self.linear.weight.data.transpose(0, 1)  # (i, o)
                step = self.lora_lsq_s.data.unsqueeze(0).repeat(self.gs, 1).transpose(0, 1).contiguous().view(weight.shape[1], weight.shape[0]).transpose(0, 1)  # (gs, o*gn) -> (o*gn, gs) -> (o, i) -> (i, o)
                qbias = self.lora_lsq_b.data.unsqueeze(0).repeat(self.gs, 1).transpose(0, 1).contiguous().view(weight.shape[1], weight.shape[0]).transpose(0, 1)
                weight = torch.div(weight - qbias, step).clamp(self.Qn, self.Qp).round() * step + qbias
                self.linear.weight.data = weight.transpose(0, 1) + (self.lora_B @ self.lora_A) * self.l_scale
                self.merged = True
        else:
            if self.w_bits == 0:
                self.merged = True
                ...
                qbias = self.lora_lsq_b.data.unsqueeze(0).repeat(self.gs, 1).transpose(0, 1).contiguous().view(weight.shape[1], weight.shape[0]).transpose(0, 1)
                weight = torch.div(weight - qbias, step).clamp(self.Qn, self.Qp).round() * step + qbias
                self.linear.weight.data = weight.transpose(0, 1)
                self.merged = True
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.w_bits == 0 or self.merged:
            return self.linear(x)
        else:
            # lora = self.lora_dropout(x) @ self.lora_A.transpose(0, 1) @ self.lora_B.transpose(0, 1) * self.l_scale
            if not self.lsq:
                pretrained = self.linear(x)
                return pretrained
            # Init lsq step s
            else:
                q_weight = WLSQPlus.apply(self.linear.weight, self.lora_lsq_s, self.lora_lsq_b, self.gs, self.g, self.Qn, self.Qp)
...
class LoRAQKVLinear(LoRALinear):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0 and any(enable_lora):
            self.lora_A = nn.Parameter(torch.zeros((r * sum(enable_lora), in_features)), requires_grad=False)  # (4, 128)
            ...
            self.lora_B = nn.Parameter(torch.zeros(sum(self.qkv_shapes), r), requires_grad=False)  # (256, 2))
        ...
        # lsq parameters
        if self.w_bits > 0:  # always true for llama 1
            self.lora_lsq_s = torch.nn.Parameter(torch.ones(3, int(in_features * in_features / gs)))
            self.lora_lsq_b = torch.nn.Parameter(torch.zeros(3, int(in_features * in_features / gs)))
            self.g = 1.0/math.sqrt(in_features * abs(self.Qp) / self.gs / 3.0)
        # self.g = 1.0/math.sqrt(out_features * in_features * self.Qp / sum(self.enable_lora))
    ...
    def merge(self) -> None:
        ...
        if self.r > 0 and any(self.enable_lora) and not self.merged:
            if self.w_bits == 0:
                ...
                # W = W + delta_W (merge)
                self.linear.weight.data += self.zero_pad(delta_w * self.l_scale)  # (256, 128) after zero_pad (384, 128)
                self.merged = True
            else:
                w = self.linear.weight.data.view(3, -1, self.linear.weight.data.shape[-1])  # (3*o, i) -> (3, o, i)
                a = self.lora_A.data.view(3, -1, self.lora_A.data.shape[-1])  # (3*r, i) -> (3, r, i)
                b = self.lora_B.data.view(3, -1, self.lora_B.data.shape[-1])  # (3*o, r) -> (3, o, r)
                weight = w.transpose(1, 2) + a.transpose(1, 2) @ b.transpose(1, 2) * self.l_scale  # (3, i, o)
                step = self.lora_lsq_s.data.unsqueeze(1).repeat(1, self.gs, 1).transpose(1, 2).contiguous().view(w.size()).transpose(1, 2)  # (3, o*gn) > (3, 1, o*gn) > (3, gs, o*gn) > (3, o*gn, gs) > (3, o, i) > (3, i, o)
                qbias = self.lora_lsq_b.data.unsqueeze(1).repeat(1, self.gs, 1).transpose(1, 2).contiguous().view(w.size()).transpose(1, 2)
                w_q = torch.div(weight - qbias, step).clamp(self.Qn, self.Qp).round() * step + qbias + a.transpose(1, 2) @ b.transpose(1, 2) * self.l_scale
                self.linear.weight.data = w_q.transpose(1, 2).contiguous().view(-1, weight.shape[1])
                self.merged = True
        else:
            if self.w_bits == 0:
                self.merged = True
                ...
                w = self.linear.weight.data.view(3, -1, self.linear.weight.data.shape[-1])  # (3*o, i) -> (3, o, i)
                weight = w.transpose(1, 2)  # (3, i, o)
                step = self.lora_lsq_s.data.unsqueeze(1).repeat(1, self.gs, 1).transpose(1, 2).contiguous().view(w.size()).transpose(1, 2)  # (3, o*gn) > (3, 1, o*gn) > (3, gs, o*gn) > (3, o*gn, gs) > (3, o, i) > (3, i, o)
                qbias = self.lora_lsq_b.data.unsqueeze(1).repeat(1, self.gs, 1).transpose(1, 2).contiguous().view(w.size()).transpose(1, 2)
                w_q = torch.div(weight - qbias, step).clamp(self.Qn, self.Qp).round() * step + qbias
                self.linear.weight.data = w_q.transpose(1, 2).contiguous().view(-1, weight.shape[1])
                self.merged = True
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.w_bits ==0 or self.merged:
            ...
            # lora = self.zero_pad(after_B) * self.l_scale  # (64, 64, 256) after zero_pad (64, 64, 384)
            if not self.lsq:
                pretrained = self.linear(x)
                return pretrained
            else:
                q_weight = WLSQPlusQKV.apply(self.linear.weight, self.lora_lsq_s, self.lora_lsq_b, self.gs, self.g, self.Qn, self.Qp)
                return F.linear(x, q_weight, self.linear.bias)
...
class Config(BaseConfig):
    ...
    def mlp_class(self) -> Type:
        return getattr(lit_gpt.qat, self._mlp_class)
...
class GPT(BaseModel):
    def __init__(self, config: Config) -> None:
        ...
        self.lsq = self.config.lsq
        self.lm_head_quant = False
        self.qb_train = True
...
class LLaMAMLP(lit_gpt.model.LLaMAMLP):
    def __init__(self, config: Config) -> None:
        ...
        self.proj = LoRALinear(
            ...
            w_bits = config.w_bits,
            q_granul = config.q_granul,
            gs = config.gs,           lora_dropout=config.dropout,
        )
...
def enable_lsq_lora(model: GPT) -> None:
    ...
    for name, module in model.named_modules():
        if not model.lm_head_quant and "lm_head" in name and isinstance(module, LoRALinear):
            ...
            if isinstance(module, LoRAQKVLinear):
                module.lora_lsq_s.data = torch.max(torch.abs((module.linear.weight).detach().contiguous().view(3, -1, module.gs)), dim=-1)[0]/module.div
            else:
                module.lora_lsq_s.data = torch.max(torch.abs((module.linear.weight).detach().contiguous().view(-1, module.gs)), dim=-1)[0]/module.div
            module.lora_lsq_s.requires_grad = True
            module.lora_lsq_b.requires_grad = True


def merge_lsq_lora_weights(model: GPT) -> None:
