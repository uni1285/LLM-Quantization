...
from lit_gpt.quant import LSQP_WQuantizer, LSQP_WQuantizerQKV
...
class LoRALinear(LoRALayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # lsq parameters
        if True:  # self.w_bits > 0:  # always true for llama 1
    ...
    def merge(self) -> None:
        ...
        if self.r > 0 and not self.merged:
            ...
            if self.w_bits == 0:
                ...
                weight = self.linear.weight.data.transpose(0, 1) + self.lora_A.data.transpose(0, 1) @ self.lora_B.data.transpose(0, 1) * self.l_scale  # (i, o)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.r == 0 or self.merged:
            ...
            # Init lsq step s
                ...
                out = LSQP_WQuantizer.apply(self.lora_dropout(x), self.linear.weight, self.linear.bias, self.lora_A, self.lora_B, self.l_scale, self.lora_lsq_s,
                                                   self.lora_lsq_b, self.gs, self.g, self.Qn, self.Qp)
                return out
...
class LoRAQKVLinear(LoRALinear):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # lsq parameters
        if True:  # self.w_bits > 0:  # always true for llama 1
            self.lora_lsq_s = torch.nn.Parameter(torch.ones(sum(enable_lora), int(in_features * in_features / gs)), requires_grad=False)
            self.lora_lsq_b = torch.nn.Parameter(torch.zeros(sum(enable_lora), int(in_features * in_features / gs)), requires_grad=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.r == 0 or not any(self.enable_lora) or self.merged:
            ...
            if not self.lsq:
                ...
                out = LSQP_WQuantizerQKV.apply(self.lora_dropout(x), self.linear.weight, self.linear.bias, self.lora_A, self.lora_B, self.l_scale, self.lora_lsq_s,
                                                   self.lora_lsq_b, self.gs, self.g, self.Qn, self.Qp, self.enable_lora)
                return out
