...
from lit_gpt.original_lora import GPT, Block, Config, lora_filter, mark_only_lora_as_trainable  # , enable_lsq_lora
...
batch_size = 128
micro_batch_size = 1
#------ SAM hyperparameter added ------
sam_rho = 0.05  # Sharpness Aware Minimization hyperparameter using L-infinity norm (only sign of gradients is used)
#------ end SAM hyperparameter ------
...
def setup(
    data_dir: Path = Path("data/alpaca_llama-2"),
    out_dir: Path = Path("out/l4q/llama-2"),
    ...
    checkpoint_dir = Path("/SHARE_ST/vlsi/anonymous/data/llama-2-" + model_size)
    plugins = None
    ...
    fabric.launch(main, data_dir, checkpoint_dir, out_dir, model_size, devices,
...
def main(fabric: L.Fabric, data_dir: Path, checkpoint_dir: Path, out_dir: Path, model_size: str, devices: int,
        ...
        if not is_accumulating:
            ...
            for p in model.parameters():
                if p.requires_grad and p.grad is not None:
                    # compute offset using only the sign of the gradient
                    offset = sam_rho * torch.sign(p.grad)
                    p.data.add_(offset)  # ascent update for sharpness-aware minimization
                    offsets[p] = offset
            #------ end SAM Ascent Step ------

            #------ SAM Descent Step ------
            logits_descent = model(input_ids, lm_head_chunk_size=128)
            logits_descent[-1] = logits_descent[-1][..., :-1, :]
            loss_descent = chunked_cross_entropy(logits_descent, targets[..., 1:])
            fabric.backward(loss_descent / gradient_accumulation_iters)
            optimizer.step()
            optimizer.zero_grad()
            #------ end SAM Descent Step ------

            #------ SAM Restoration Step ------
            for p, offset in offsets.items():
                p.data.sub_(offset)  # restore original parameter values after descent step
            #------ end SAM Restoration Step ------
