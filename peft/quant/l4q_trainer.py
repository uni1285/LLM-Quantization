...
class L4QTrainer(Trainer):
    def __init__(
        self,
        model: Union[PreTrainedModel, nn.Module] = None,
        args: TrainingArguments = None,
        l4q_start_steps: Optional[int] = None,
        data_collator: Optional[DataCollator] = None,
        train_dataset: Optional[Union[Dataset, IterableDataset, "datasets.Dataset"]] = None,
        eval_dataset: Optional[Union[Dataset, Dict[str, Dataset], "datasets.Dataset"]] = None,
        tokenizer: Optional[PreTrainedTokenizerBase] = None,
        ...
        ## modification for L4Q ##
        self.l4q_start_steps = 0 if l4q_start_steps is None else l4q_start_steps
        ##########################
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if step % args.gradient_accumulation_steps == 0:
                    self.control = self.callback_handler.on_step_begin(args, self.state, self.control)
                    ## modification for L4Q ##
                    if step // args.gradient_accumulation_steps == self.l4q_start_steps:
                        self.enable_l4q()
                    ##########################
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            logs["learning_rate"] = self._get_learning_rate()
            ## modification for L4Q ##
            logs["Memory"] = round(torch.cuda.max_memory_allocated() / 1e9, 2)
            ##########################
    ...
    def enable_l4q(self):
        print("*** Switch to L4Q training ***")
        from peft.tuners.lora.layer import L4Q_Linear
        for name, module in self.model.named_modules():
            if isinstance(module, L4Q_Linear):
                if not module.l4q:
                    module.l4q = True
                    module.reset_l4q_parameters("default")
                    module.lora_lsq_s.requires_grad = True
                    module.lora_lsq_b.requires_grad = True


def L4QForceInit(model, adapter_name: "default", config: 0):
    print("*** L4Q ignore loaded model and revert to init state ***")
    from peft.tuners.lora.layer import L4Q_Linear
    q_error_sum = torch.tensor([0.])
    for name, module in model.named_modules():
        if isinstance(module, L4Q_Linear):
            # module.lora_B[adapter_name].weight.data = torch.zeros_like(module.lora_B[adapter_name].weight.data)
            # module.reset_l4q_parameters(adapter_name)
            q_error_sum += module.get_post_error(adapter_name, config)
    # return model
    return q_error_sum


def L4QProfiler(model, adapter_name: "default", config: 0):
    print("*** L4Q init quantization error profiling ***")
    from peft.tuners.lora.layer import L4Q_Linear
    q_error_sum = torch.tensor([0., 0., 0., 0.])
    for name, module in model.named_modules():
        if isinstance(module, L4Q_Linear):
            q_error_sum += module.get_pre_error(adapter_name, config)
    return q_error_sum


def enable_lora(model):
    print("*** Enable LoRA fine-tuning ***")
    from peft.tuners.lora.layer import L4Q_Linear
    for name, module in model.named_modules():
            module.lora_lsq_s.requires_grad = True
            module.lora_lsq_b.requires_grad = True
