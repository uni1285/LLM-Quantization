...
from lit_llama.lora import LLaMA, Block, Config, merge_lsq_lora_weights
...
from evaluate.utils import dt_analysis, loss_landscape, cos_similarity
...
def main(
    ...
    cos_similar_path: Path = Path("out/model.pth"),
    analysis: bool = False,
    ...
    if checkpoint_dir is None:
        checkpoint_dir = Path("/SHARE_ST/vlsi/hyesungjeon/data/llama-" + model_size)
    ...
    if similarity:
        ...
        t_checkpoint = lazy_load(checkpoint_path)
        t_lora_checkpoint = lazy_load(cos_similar_path)
        t_checkpoint.update(t_lora_checkpoint.get("model", t_lora_checkpoint))
        target.load_state_dict(t_checkpoint)
        merge_lsq_lora_weights(target)

        cos_similarity(target=target, model=model)
